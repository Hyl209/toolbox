...
class Base64Service:
    ...
    def encode(self, file_path: str | Path, data_url: bool = False) -> str:
        """文件转 Base64"""
        try:
            conv = self._get_converter()
            encoded = conv.encode_file_to_base64(file_path)
            if data_url:
                encoded = conv.build_data_url(encoded, Path(file_path).suffix)
            return encoded
